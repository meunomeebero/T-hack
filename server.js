...
const { DB_PATH, addWin, closeDatabase, getTopRankings } = require("./db");

const PORT = process.env.PORT || 3000;
const GAME_ROUNDS = 5;
const ROUND_TIME_LIMIT_MS = 30000;
...
function handleGetRankings(ws) {
  send(ws, { type: "rankings", rankings: getTopRankings(10) });
}
...
function generateCommands(count) {
  ...
  const shuffled = allCommands.slice().sort(() => Math.random() - 0.5);
